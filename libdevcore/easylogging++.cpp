...
static const char* kMonths[12] = {"January", "February", "March", "Apri", "May", "June", "July",
...
{
    // Do not use switch over strongly typed enums because Intel C++ compilers dont support them
    // yet.
...
static struct StringToLevelItem stringToLevelMap[] = {{"global", Level::Global},
    {"debug", Level::Debug}, {"info", Level::Info}, {"warning", Level::Warning},
    {"error", Level::Error}, {"fatal", Level::Fatal}, {"verbose", Level::Verbose},
    {"trace", Level::Trace}};
...
void Configuration::log(el::base::type::ostream_t& os) const
{
    os << LevelHelper::convertToString(m_level) << ELPP_LITERAL(" ")
       << ConfigurationTypeHelper::convertToString(m_configurationType) << ELPP_LITERAL(" = ")
       << m_value.c_str();
}
...
Configurations::Configurations(
    const std::string& configurationFile, bool useDefaultsForRemaining, Configurations* base)
...
{
    base::threading::ScopedLock scopedLock(lock());
    unsafeSet(level, configurationType, value);  // This is not unsafe anymore as we have locked
                                                 // mutex
    if (level == Level::Global)
    {
...
void Configurations::setRemainingToDefault(void)
{
...
#endif  // defined(ELPP_NO_LOG_TO_FILE)
    unsafeSetIfNotExist(
        Level::Global, ConfigurationType::Filename, std::string(base::consts::kDefaultLogFile));
    ...
    unsafeSetIfNotExist(
        Level::Global, ConfigurationType::Format, std::string("%datetime %level [%logger] %msg"));
    unsafeSetIfNotExist(Level::Debug, ConfigurationType::Format,
        std::string("%datetime %level [%logger] [%user@%host] [%func] [%loc] %msg"));
    // INFO and WARNING are set to default by Level::Global
    unsafeSetIfNotExist(
        Level::Error, ConfigurationType::Format, std::string("%datetime %level [%logger] %msg"));
    unsafeSetIfNotExist(
        Level::Fatal, ConfigurationType::Format, std::string("%datetime %level [%logger] %msg"));
...
bool Configurations::Parser::parseFromFile(
...
bool Configurations::Parser::parseLine(std::string* line, std::string* currConfigStr,
...
void Configurations::unsafeSetIfNotExist(
    Level level, ConfigurationType configurationType, const std::string& value)
{
    Configuration* conf =
        RegistryWithPred<Configuration, Configuration::Predicate>::get(level, configurationType);
    if (conf == nullptr)
    {
...
void Configurations::setGlobally(
...
void Configurations::unsafeSetGlobally(
    ConfigurationType configurationType, const std::string& value, bool includeGlobalLevel)
{
    if (includeGlobalLevel)
    {
...
base::type::fstream_t* File::newFileStream(const std::string& filename)
{
    base::type::fstream_t* fs =
        new base::type::fstream_t(filename.c_str(), base::type::fstream_t::out
...
{
    str.erase(
        str.begin(), std::find_if(str.begin(), str.end(), [](char c) { return !std::isspace(c); }));
    return str;
}
    ...
    const std::string& replaceWith)
{
    replaceFirstWithEscape(
        str, replaceWhat, base::type::string_t(replaceWith.begin(), replaceWith.end()));
}
#endif  // defined(ELPP_UNICODE)
...
char* Str::convertAndAddToBuff(
    std::size_t n, int len, char* buf, const char* bufLim, bool zeroPadded)
{
    char localBuff[10] = "";
...
std::string OS::getEnvironmentVariable(
    const char* variableName, const char* defaultVal, const char* alternativeBashCommand)
{
#if ELPP_OS_UNIX
...
bool OS::termSupportsColor(void)
{
    ...
    return term == "xterm" || term == "xterm-color" || term == "xterm-256color" ||
           term == "screen" || term == "linux" || term == "cygwin" || term == "screen-256color";
}
    ...
    struct timeval tval, const char* format, const el::base::SubsecondPrecision* ssPrec)
{
    ...
    parseFormat(buff_, kBuffSize, format, &timeInfo,
        static_cast<std::size_t>(tval.tv_usec / ssPrec->m_offset), ssPrec);
    return std::string(buff_);
}

base::type::string_t DateTime::formatTime(
...
struct ::tm* DateTime::buildTimeInfo(struct timeval* currTime, struct ::tm* timeInfo)
{
    ...
    return timeInfo;
#else
    // For any other compilers that don't have CRT warnings issue e.g, MinGW or TDM GCC- we use
    // different method
    time_t rawTime = currTime->tv_sec;
    ...
    const struct tm* tInfo, std::size_t msec, const base::SubsecondPrecision* ssPrec)
{
    ...
    for (; *format; ++format)
    {
        if (*format == base::consts::kFormatSpecifierChar)
        {
            ...
            case 'B':  // month (long)
                buf =
                    base::utils::Str::addToBuff(base::consts::kMonths[tInfo->tm_mon], buf, bufLim);
                continue;
...
bool LogFormat::operator==(const LogFormat& other)
{
    return m_level == other.m_level && m_userFormat == other.m_userFormat &&
           m_format == other.m_format && m_dateTimeFormat == other.m_dateTimeFormat &&
...
void LogFormat::parseFromFormat(const base::type::string_t& userFormat)
{
        ...
        while ((foundAt = formatCopy.find(specifier, foundAt + 1)) != base::type::string_t::npos)
        {
            if (foundAt > 0 && formatCopy[foundAt - 1] == base::consts::kFormatSpecifierChar)
            {
                if (hasFlag(flag))
                {
                    ...
                    // '%' even after specifier resolution - this is because we only replaceFirst
                    // specifier
    ...
    conditionalAddFlag(
        base::consts::kSeverityLevelShortFormatSpecifier, base::FormatFlags::LevelShort);
...
void LogFormat::updateFormatSpec(void)
{
    ...
    {
        base::utils::Str::replaceFirstWithEscape(m_format,
            base::consts::kSeverityLevelFormatSpecifier, base::consts::kDebugLevelLogValue);
        base::utils::Str::replaceFirstWithEscape(m_format,
            base::consts::kSeverityLevelShortFormatSpecifier,
            base::consts::kDebugLevelShortLogValue);
    }
    else if (m_level == Level::Info)
    {
        base::utils::Str::replaceFirstWithEscape(m_format,
            base::consts::kSeverityLevelFormatSpecifier, base::consts::kInfoLevelLogValue);
        base::utils::Str::replaceFirstWithEscape(m_format,
            base::consts::kSeverityLevelShortFormatSpecifier,
    ...
    {
        base::utils::Str::replaceFirstWithEscape(m_format,
            base::consts::kSeverityLevelFormatSpecifier, base::consts::kWarningLevelLogValue);
        base::utils::Str::replaceFirstWithEscape(m_format,
            base::consts::kSeverityLevelShortFormatSpecifier,
    ...
    {
        base::utils::Str::replaceFirstWithEscape(m_format,
            base::consts::kSeverityLevelFormatSpecifier, base::consts::kErrorLevelLogValue);
        base::utils::Str::replaceFirstWithEscape(m_format,
            base::consts::kSeverityLevelShortFormatSpecifier,
    ...
    {
        base::utils::Str::replaceFirstWithEscape(m_format,
            base::consts::kSeverityLevelFormatSpecifier, base::consts::kFatalLevelLogValue);
        base::utils::Str::replaceFirstWithEscape(m_format,
            base::consts::kSeverityLevelShortFormatSpecifier,
    ...
    {
        base::utils::Str::replaceFirstWithEscape(m_format,
            base::consts::kSeverityLevelFormatSpecifier, base::consts::kVerboseLevelLogValue);
        base::utils::Str::replaceFirstWithEscape(m_format,
            base::consts::kSeverityLevelShortFormatSpecifier,
    ...
    {
        base::utils::Str::replaceFirstWithEscape(m_format,
            base::consts::kSeverityLevelFormatSpecifier, base::consts::kTraceLevelLogValue);
        base::utils::Str::replaceFirstWithEscape(m_format,
            base::consts::kSeverityLevelShortFormatSpecifier,
...
void TypedConfigurations::build(Configurations* configurations)
{
    ...
    for (Configurations::const_iterator it = configurations->begin(); it != configurations->end();
         ++it)
    {
    ...
    // As mentioned earlier, we will now set filename configuration in separate loop to deal with
    // non-existent files
    for (Configurations::const_iterator it = configurations->begin(); it != configurations->end();
         ++it)
    {
         ...
         conf != withFileSizeLimit.end(); ++conf)
    {
        ...
        unsafeValidateFileRolling((*conf)->level(), ELPP->preRollOutCallback());
    }
}
...
std::string TypedConfigurations::resolveFilename(const std::string& filename)
{
    ...
    if ((dateIndex = resultingFilename.find(dateTimeFormatSpecifierStr.c_str())) !=
        std::string::npos)
    {
        while (
            dateIndex > 0 && resultingFilename[dateIndex - 1] == base::consts::kFormatSpecifierChar)
        {
        ...
        if (dateIndex != std::string::npos)
        {
            ...
            base::utils::Str::replaceAll(now, '/', '-');  // Replace path element since we are
...
void TypedConfigurations::insertFile(Level level, const std::string& fullFilename)
{
    ...
    {
        std::cerr << "Could not load empty file for logging, please re-check your configurations "
                     "for level ["
    ...
    auto create = [&](Level level) {
        ...
        if (needNewStream)
        {
            ...
            m_logStreamsReference->insert(std::make_pair(
                resolvedFilename, base::WeakFileStreamPtr(m_fileStreamMap.at(level))));
        }
        else
        {
            ...
            m_fileStreamMap.insert(
                std::make_pair(level, base::FileStreamPtr(filestreamIter->second.lock())));
        ...
        if (fs == nullptr)
        {
            ...
            ELPP_INTERNAL_ERROR(
                "Setting [TO_FILE] of [" << LevelHelper::convertToString(level) << "] to FALSE",
                false);
    ...
    Level level, const PreRollOutCallback& preRollOutCallback)
{
    ...
    if (maxLogFileSize != 0 && currFileSize >= maxLogFileSize)
    {
        ...
        ELPP_INTERNAL_INFO(1, "Truncating log file ["
                                  << fname << "] as a result of configurations for level ["
                                  << LevelHelper::convertToString(level) << "]");
        fs->close();
...
bool RegisteredHitCounters::validateEveryN(
...
/// @brief Validates counter for hits >= N, i.e, registers new if does not exist otherwise updates
/// original one
/// @return True if validation resulted in triggering hit. Meaning logs should be written everytime
/// true is returned
bool RegisteredHitCounters::validateAfterN(
    const char* filename, base::type::LineNumber lineNumber, std::size_t n)
{
    base::threading::ScopedLock scopedLock(lock());
    base::HitCounter* counter = get(filename, lineNumber);
...
/// @return True if validation resulted in triggering hit. Meaning logs should be written everytime
/// true is returned
bool RegisteredHitCounters::validateNTimes(
    const char* filename, base::type::LineNumber lineNumber, std::size_t n)
{
    base::threading::ScopedLock scopedLock(lock());
    base::HitCounter* counter = get(filename, lineNumber);
...
void VRegistry::setFromArgs(const base::utils::CommandLineArgs* commandLineArgs)
{
    ...
    {
        setLevel(
            static_cast<base::type::VerboseLevel>(atoi(commandLineArgs->getParamValue("--v"))));
    }
    else if (commandLineArgs->hasParamWithValue("--V"))
    {
        setLevel(
    ...
    m_preRollOutCallback(base::defaultPreRollOutCallback)
{
    ...
    sysLogLogger->configurations()->setGlobally(
        ConfigurationType::Format, std::string("%level: %msg"));
...
bool Storage::uninstallCustomFormatSpecifier(const char* formatSpecifier)
{
    ...
    std::vector<CustomFormatSpecifier>::iterator it = std::find(
        m_customFormatSpecifiers.begin(), m_customFormatSpecifiers.end(), formatSpecifier);
...
void LogDispatchCallback::handle(const LogDispatchData* data)
{
    ...
    std::string filename =
        data->logMessage()->logger()->typedConfigurations()->filename(data->logMessage()->level());
    auto lock = m_fileLocks.find(filename);
    if (lock == m_fileLocks.end())
    {
        m_fileLocks.emplace(std::make_pair(
            filename, std::unique_ptr<base::threading::Mutex>(new base::threading::Mutex)));
    }
#endif
}
...
base::type::string_t DefaultLogBuilder::build(
    const LogMessage* logMessage, bool appendNewLine) const
{
    ...
    if (logFormat->hasFlag(base::FormatFlags::Line))
    {
        ...
        buf = base::utils::Str::convertAndAddToBuff(
            logMessage->line(), base::consts::kSourceLineMaxLength, buf, bufLim, false);
        base::utils::Str::replaceFirstWithEscape(
        ...
        logFormat->hasFlag(base::FormatFlags::VerboseLevel))
    {
        ...
        buf = base::utils::Str::convertAndAddToBuff(
            logMessage->verboseLevel(), 1, buf, bufLim, false);
        base::utils::Str::replaceFirstWithEscape(
...
void LogDispatcher::dispatch(void)
{
    ...
    {
        // modify by liangyongfu 按大小切换日志，统一放在定时线程中，避免多线程处理core
...
PerformanceTracker::PerformanceTracker(const std::string& blockName,
    base::TimestampUnit timestampUnit, const std::string& loggerId, bool scopedLog, Level level)
    ...
    m_enabled(false)
{
    ...
    // or before calling checkpoint, we still depend on state of configuraton at time of
    // construction
    el::Logger* loggerPtr = ELPP->registeredLoggers()->get(loggerId, false);
    m_enabled =
        loggerPtr != nullptr && loggerPtr->m_typedConfigurations->performanceTracking(m_level);
    if (m_enabled)
    {
...
void PerformanceTracker::checkpoint(
    const std::string& id, const char* file, base::type::LineNumber line, const char* func)
{
#if !defined(ELPP_DISABLE_PERFORMANCE_TRACKING) && ELPP_LOGGING_ENABLED
    if (m_enabled)
    {
...
const base::type::string_t PerformanceTracker::getFormattedTimeTaken(struct timeval startTime) const
{
    ...
    return base::utils::DateTime::formatTime(
        base::utils::DateTime::getTimeDifference(m_endTime, startTime, m_timestampUnit),
        m_timestampUnit);
}
...
StackTrace::StackTraceEntry::StackTraceEntry(std::size_t index, const std::string& loc,
    const std::string& demang, const std::string& hex, const std::string& addr)
...
std::ostream& operator<<(std::ostream& ss, const StackTrace::StackTraceEntry& si)
{
    ss << "[" << si.m_index << "] " << si.m_location << (si.m_hex.empty() ? "" : "+") << si.m_hex
       << " " << si.m_addr << (si.m_demangled.empty() ? "" : ":") << si.m_demangled;
    return ss;
}
...
static std::string crashReason(int sig)
{
    ...
    for (int i = 0; i < base::consts::kCrashSignalsCount; ++i)
    {
        ...
        {
            ss << "Application has crashed due to [" << base::consts::kCrashSignals[i].name
               << "] signal";
...
///
/// @detail This function writes log using 'default' logger, prints stack trace for GCC based
/// compilers and aborts program.
...
Logger* Loggers::reconfigureLogger(
...
void Loggers::reconfigureAllLoggers(
    Level level, ConfigurationType configurationType, const std::string& value)
{
    for (base::RegisteredLoggers::iterator it = ELPP->registeredLoggers()->begin();
         it != ELPP->registeredLoggers()->end(); ++it)
...
void Loggers::setDefaultConfigurations(
    const Configurations& configurations, bool reconfigureExistingLoggers)
{
...
void Loggers::configureFromGlobal(const char* globalConfigurationFilePath)
{
    ...
    ELPP_ASSERT(gcfStream.is_open(), "Unable to open global configuration file ["
                                         << globalConfigurationFilePath << "] for parsing.");
    std::string line = std::string();
    std::stringstream ss;
    ...
    while (gcfStream.good())
    {
        ...
        base::utils::Str::trim(line);
        if (line.size() > 2 &&
            base::utils::Str::startsWith(line, std::string(base::consts::kConfigurationLoggerId)))
        {
