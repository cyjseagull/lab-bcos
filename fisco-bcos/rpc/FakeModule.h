...
public:
    MockService() : Service()
    {
        dev::p2p::NodeID nodeID = h512(100);
        dev::p2p::NodeIPEndpoint m_endpoint(bi::address::from_string("127.0.0.1"), 30303, 30310);
        dev::p2p::SessionInfo info(nodeID, m_endpoint, std::set<std::string>());
        std::set<std::string> topics;
        std::string topic = "Topic1";
        topics.insert(topic);
        m_sessionInfos.push_back(dev::p2p::SessionInfo(nodeID, m_endpoint, topics));
    }

    virtual dev::p2p::SessionInfos sessionInfos() override { return m_sessionInfos; }
    void setSessionInfos(dev::p2p::SessionInfos& sessionInfos) { m_sessionInfos = sessionInfos; }
    void appendSessionInfo(dev::p2p::SessionInfo const& info) { m_sessionInfos.push_back(info); }
    void clearSessionInfo() { m_sessionInfos.clear(); }
    dev::p2p::SessionInfos sessionInfosByProtocolID(dev::PROTOCOL_ID _protocolID) const
    {
    ...
    dev::p2p::Message::Ptr getAsyncSendMessageByNodeID(dev::p2p::NodeID const& nodeID)
    {
